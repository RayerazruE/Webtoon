...
<!-- 				</div> -->
				...
				<c:if test="${cmtList.user_id == cmt_user_id}">
				...
				<input type="hidden" value="<c:out value="${User_id}"/>">
				 <c:set var="users_ids" value="${users_id}" />
				...
				<c:if test="${cmtList.user_id == cmt_user_id}">
					...
					<c:set var="Webtoon_id" value="${webtoon_id}" />
					<button id="cmt_${loop.index}" value="<c:out value="${loop.index}"/>" class="btn btn-danger mt-auto" onclick="admindeleteComment(${loop.index})"><c:out value="${cmtList.user_id}"/></button>
...
<script>
	...
	//var users_id = ${users_id}
	 ...
	 function admindeleteComment(indx) {
		
		 var indx = "${index}";
		 
		var users_id = $('#users_id').val();
		 
		 alert("Button " + indx + "이 클릭되었습니다.");
